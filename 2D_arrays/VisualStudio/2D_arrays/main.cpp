...
//        23 13 03
 ...
 /*     По данным числам n и m заполните двумерный массив размером n×m числами от 1 до n×m по спирали,
      ...
      //РЕШЕНИЕ
      /*Достаточно одного цикла while () и направления обхода, на каждом шаге 
      проверяем не вышли ли мы за границы и равен ли нулю следующий элемент, 
      если какое то условие не выполнилось, то меняем направление обхода по часовой стрелке.
      если мы 4 раза сменили направление обхода и идти некуда, то это конец, весь массив заполнен)
      если не очень красиво объяснил можете про метод щепина распознавания образов почитать, там подробно описано.*/
